...
import { createPrajurit, uploadAvatar } from "../action";
...
export default function InputPrajurit() {
  ...
  const onSubmit = async (payload: InputPrajuritSchemaType) => {
    setIsLoading(true);
    if (files.length > 0) {
      payload.avatar = await uploadAvatar(files[0], payload.username);
    }

    const result = await createPrajurit(payload);
  ...
  return (
    <div className="w-full flex justify-center">
      ...
      <NotificationDialog
        ...
        title="Gagal Menyimpan Prajurit!"
