...
import { updateBerita, uploadThumbnail } from "../../action";

type TBerita = Awaited<ReturnType<typeof getBeritaById>>;

export default function EditBeritaForm({ berita }: { berita: TBerita }) {
  if (!berita) {
    return null;
  }
  ...
  const form = useForm<UpdateBeritaSchemaType>({
    resolver: zodResolver(UpdateBeritaSchema),
    defaultValues: {
      id: berita.id,
      judul: berita.judul,
      deskripsi: berita.deskripsi as string,
  ...
  const onSubmit = async (payload: UpdateBeritaSchemaType) => {
    setIsLoading(true);
    ...
    const result = await updateBerita(payload);
