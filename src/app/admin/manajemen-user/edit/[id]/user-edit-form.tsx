...
import { uploadAvatar } from "@/app/admin/prajurit/action";
import {
  UpdateUserSchema,
...
interface PrajuritEditFormProps {
  userData: (User & { profile: Profile | null }) | null;
  userId: string;
}

export function PrajuritEditForm({ userData, userId }: PrajuritEditFormProps) {
  ...
  const form = useForm<UpdateUserSchemaType>({
    ...
    defaultValues: {
      ...
      role: userData?.role,
      name: userData?.name || "",
      pangkat: userData?.profile?.pangkat || "",
      jenis_kelamin: userData?.profile?.jenisKelamin || "",
      umur: userData?.profile?.umur || 0,
      golongan: userData?.profile?.golongan || 0,
  ...
  const [errorMessage, setErrorMessage] = useState(
    "Terjadi kesalahan saat memperbarui data user. Silakan coba lagi."
  );
  ...
  useEffect(() => {
    if (userData?.avatar) {
      setInitialAvatarUrl(userData.avatar);
    }
  }, [userData?.avatar]);
  ...
  if (!userData) {
    return (
      <NotificationDialog
        isOpen={true}
        onClose={() => router.push("/admin/manajemen-user")}
